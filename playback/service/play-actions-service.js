import {
  isCommandExcluded,
  isSelfHealingEnable,
} from "../../panel/js/UI/services/self-healing-service/utils.js";
...
function playSuite(i) {
  ...
  if (i < length) {
    selectedCaseIndex = i;
    logger.info(
      "Playing test case: " + testSuiteData[selectedCaseIndex]["testCaseName"]
    );
    socketLog.emit("logger", {
      mess:
        "Playing test case: " +
        testSuiteData[selectedCaseIndex]["testCaseName"],
      type: "verbose",
    });
    //logStartTime();
    logger.logTime();
...
function executionLoop() {
  let commands = getRecordsArray(
    testSuiteData[selectedCaseIndex]["testCaseHTML"]
  );
  ...
  console.log(currentPlayingCommandIndex, commands[currentPlayingCommandIndex]);
  ...
  if (isExtCommand(commandName)) {
    ...
    return extCommand["do" + upperCase](commandTarget, commandValue).then(
      executionLoop
    );
  } else {
...
async function doCommand() {
  ...
  let commandTarget = getCommandTarget(commands[currentPlayingCommandIndex]);
  let commandValue = getCommandValue(commands[currentPlayingCommandIndex]);
  possibleTargets = await getPossibleTargetList(
    commands[currentPlayingCommandIndex]
  );
...
async function runCommand(commands, commandName, commandTarget, commandValue) {
  if (commandName.indexOf("${") !== -1) {
    ...
    logger.info(
      "Expand variable '" + originalVariable + "' into '" + commandName + "'"
    );
    socketLog.emit("logger", {
      mess:
        "Expand variable '" + originalVariable + "' into '" + commandName + "'",
  ...
  return p
    .then(async function () {
      ...
      if (commandName === "if") {
        let condition = await evalIfCondition(commandTarget, declaredVars);
        blockStack.push({
          isIf: true,
          condition: condition,
      ...
      if (commandName === "elseIf") {
        if (lastBlock.met) {
          ...
          lastBlock.condition = await evalIfCondition(
            commandTarget,
            declaredVars
          );
          lastBlock.met = lastBlock.condition;
      ...
      if (commandName === "while") {
        blockStack.push({
          ...
          condition: await evalIfCondition(commandTarget, declaredVars),
      ...
      if (commandName === "gotoIf") {
        if (await evalIfCondition(commandTarget, declaredVars)) {
      ...
      if (commandName === "storeCsv") {
        ...
        let csvValue = dataService.parseData(tokens[0]).data[
          parseInt(tokens[1])
        ][tokens[2]];
...
function handleFormatCommand(message, sender, response) {
  if (message.storeVar !== undefined) {
    logger.info(
      "Store '" + message.storeStr + "' into '" + message.storeVar + "'"
    );
    socketLog.emit("logger", {
...
export { setTestSuiteData, playSuiteAction, setDataService };
