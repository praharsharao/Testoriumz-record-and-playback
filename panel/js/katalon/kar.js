...
function addContextMenuButton(id, node, menu, type) {
  ...
  buttonWrapper.innerHTML =
    '<button class="btn-more tooltip"><img src="/katalon/images/SVG/more-icon.svg" alt="More"><span class="tooltiptext" style="width: 40px;margin-left: -15px;">More</span></button>';
...
$(function () {
  $("#record").attr(
    "title",
                    "Click and navigate to the desired browser tab and record your tests. NOTE: If the tab has been opened before Testoriumz Recorder was installed, please refresh it."
  );
  $("#playback").attr(
    "title",
                    "Run selected test case on the active tab, any interference may stop the process. NOTE: If the tab has been opened before Testoriumz Recorder was installed, please refresh it."
  );
  $("#playSuite").attr(
    "title",
                    "Run selected test suite on the active tab, any interference may stop the process. NOTE: If the tab has been opened before Testoriumz Recorder was installed, please refresh it."
  );
  $("#playSuites").attr(
    "title",
                    "Run all test suites on the active tab, any interference may stop the process. NOTE: If the tab has been opened before Testoriumz Recorder was installed, please refresh it."
  );
  $("#extended-features").attr("title", "Daily usage");
  $("#settings").attr("title", "Settings");
  $(".sub_btn#help").attr("title", "Help");
  $("#selectElementButton").attr(
    "title",
    "Select a target element for the current command"
  );
  $("#showElementButton").attr(
    "title",
    "Find and highlight the curent target element of the current command"
  );
...
$(function () {
  ...
  speedButton.on("click", function (event) {
    event.stopPropagation();
...
$(function () {
  $(document).attr("title", "Testoriumz Recorder " + manifestData.version);
});
...
function switchRecordButton(stop) {
  ...
  if (stop) {
    ...
    record
      .find("img")
      .attr("src", "/katalon/images/SVG/record-stop-icon-16.svg");
...
function isVarName(name) {
  ...
  return name.match("^([a-zA-Z_$][a-zA-Zd_$]*)$");
}
