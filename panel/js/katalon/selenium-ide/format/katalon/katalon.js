...
this.katalonStudioStoreCSVFile = {};
...
function convertLoadVars(expression, command) {
  const dataFileName = command.target;
  let dataVariableName;

  if (window.dataFiles[dataFileName] === undefined) {
    return "//This test case requires a data file to be convert correctly";
  }

  if (katalonStudioStoreCSVFile[dataFileName]) {
    dataVariableName = katalonStudioStoreCSVFile[dataFileName];
  } else {
    dataVariableName = `data_${Object.keys(katalonStudioStoreCSVFile).length}`;
    katalonStudioStoreCSVFile[dataFileName] = dataVariableName;
  }
  let result = `CSVData ${dataVariableName} = findTestData("${dataFileName}")\n`;
  result += `for (def index in (0..${dataVariableName}.getRowNumbers() - 1)) {\n`

  if (window.dataFiles[dataFileName].data === undefined) {
    parseData(dataFileName);
  }
  if (window.dataFiles[dataFileName].data[0] === undefined) {
    return result;
  }
  const columnNames = Object.keys(dataFiles[dataFileName].data[0]);

  for (const columnName of columnNames) {
    addDeclaredVar(columnName);
    result += `def ${columnName} = ${dataVariableName}.internallyGetValue("${columnName}", index)\n`;
  }
  return result;

}

function convertSendKeys(expression, command) {
  const regex = /(?<=\${KEY_).+?(?=})/g;
  const keys = command.value?.match(regex);
  const key = keys ? keys[0] : undefined;
  switch (key) {
    case "ENTER":
      expression.args[1] = "Keys.ENTER";
      break;
    case "SHIFT":
      expression.args[1] = "Keys.SHIFT";
      break;
    case "CTRL":
      expression.args[1] = "Keys.CONTROL";
      break;
  }
...
function statement(expression, command) {
  ...
  switch (command?.command) {
    ...
    case "loadVars":
      return convertLoadVars(expression, command);
    case "endLoadVars":
      return "}"
    case "sendKeys":
      return convertSendKeys(expression, command);
    default:
      return expression.toString(command);
  }
}
