/**
 * @type { typeof import("../UI/services/tracking-service/segment-tracking-service").Tracker }
 */
let Tracker;
(async () => {
  const trackings = await import(
    "../UI/services/tracking-service/segment-tracking-service.js"
  );
  Tracker = trackings.Tracker;
})();
  ...
  Object.keys(extensions).forEach(async function (extensionFileName) {
    ...
    const info = await sandboxEvaluator.eval(`(() => {
      class LocatorBuilders {
        static builders = [];
        static add(name, builder) {
          this.builders.push({ name, builder: builder.toString() });
        }
      ...
      return {
        builders: LocatorBuilders.builders,
        _preferredOrder: LocatorBuilders._preferredOrder,
        Selenium: Selenium,
      }
    })()`);
    Object.entries(info.Selenium.prototype).forEach(([key, value]) => {
      Selenium.prototype[key] = () => {
        return sandboxEvaluator.eval(value);
      };
    });
    console.log(`>>> ${extensionFileName}:`, info);
  });
    ...
    rename.addEventListener(
      ...
      function (event) {
        ...
        const newName = prompt(
          "Please enter the new name for this extension script.",
          oldName
        );
  ...
  function renderDownloadListItem(name) {
    ...
    download.addEventListener(
      ...
      function (event) {
        event.stopPropagation();
        let fileContent =
          "data:text/javascript;charset=utf-8," + extensions[name].content;
...
$(function () {
  ...
  document.getElementById("load-extension-hidden").addEventListener(
    "change",
    function (event) {
      event.stopPropagation();
      ...
      this.value = null;
      Tracker.uploadKeyword("explorer-section-action");
    },
    false
  );
});
