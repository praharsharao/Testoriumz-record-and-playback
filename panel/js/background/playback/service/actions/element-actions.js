...
const showElementAction = () => {
  try {
    let targetValue = $('td.selectedTd').find('#command-target').val();
    if (targetValue === "auto-located-by-tac" || $('td.selectedTd').find('#command-target').length == 0) {
      targetValue = $('td.selectedTd').children().eq(1).text();
    }
