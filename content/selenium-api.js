...
Selenium.prototype.doPrePageWait = function() {
    window.sideex_new_page = (function() {return window.new_page;}());
...
Selenium.prototype.doPageWait = function () {
    ...
    window.sideex_page_done = (function() {
        if (window.document.readyState == "complete") {
            return true;
        } else {
            return false;
...
Selenium.prototype.doAjaxWait = function() {
    window.sideex_ajax_done = (function() {
        // check ajax wait
        if (window.ajax_obj) {
...
Selenium.prototype.doDomWait = function() {
    window.sideex_dom_time = (function() {return window.domModifiedTime;}());
                    ...
                    function () {
                        $.ajax({
                            ...
                            error: function (jqXHR, textStatus, errorThrown) {
                                reject(`${textStatus} - If you are using Firefox, please check if Testoriumz Recorder helper has started.`)
                            }
                        });
                    },
                    500
...
Selenium.prototype.getEval = function (script) {
    ...
    try {
        ...
        var result = eval(trustedPolicy.createScript(script));
...
Selenium.prototype.getAttributeFromAllWindows = function (attributeName) {
    ...
    try {
        attributes.push(eval(trustedPolicy.createScript("win." + attributeName)));
    } catch (ignored) {
        // Dead object
    }
...
Selenium.prototype.findWindow = function (soughtAfterWindowPropertyValue) {
    ...
    // but in this case, attributeName may contain dots (e.g. document.title)
    // in that case, we have no choice but to use eval...
    if (PatternMatcher.matches(soughtAfterWindowPropertyValue, eval(trustedPolicy.createScript("this.browserbot.topWindow." + targetPropertyName)))) {
    ...
    for (windowName in selenium.browserbot.openedWindows) {
        ...
        if (PatternMatcher.matches(soughtAfterWindowPropertyValue, eval(trustedPolicy.createScript("openedWindow." + targetPropertyName)))) {
            return openedWindow;
        }
    }
...
Selenium.prototype.doDragAndDropToObjectByJqueryUI = function (locatorOfObjectToBeDragged, locatorOfDragDestinationObject) {
    this.doDragAndDropToObject(locatorOfObjectToBeDragged, locatorOfDragDestinationObject)
...
Selenium.prototype.doWaitForCondition = function (script, timeout) {
    ...
    return Selenium.decorateFunctionWithTimeout(function () {
        ...
        return eval(trustedPolicy.createScript(script));
    }, timeout);
};
...
Selenium.prototype.doAssertConfirmationPresent = function (value) {
    this.doAssertConfirmation(value);
};

Selenium.prototype.doVerifyConfirmationPresent = function (value) {
    this.doAssertConfirmation(value);
};
