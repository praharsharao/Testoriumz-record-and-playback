...
BrowserBot.prototype.selectWindow = function(target) {
    ...
    // TODO separate name and var into separate functions
        ...
        var win = this.getCurrentWindow().eval(trustedPolicy.createScript(locatorValue));
        if (win) {
...
BrowserBot.prototype.doesThisFrameMatchFrameExpression = function(currentFrameString, target) {
    ...
    if (target == "relative=up") {
        if (currentFrame.window.parent == autWindow || currentFrame.window.parent == autWindow.originalWindow) {
            return true;
        }
        return false;
    }
    if (target == "relative=top") {
        if (currentFrame.window.top == autWindow || currentFrame.window.top == autWindow.originalWindow) {
            return true;
        }
        return false;
    }
    ...
    if (browserVersion.isHTA) {
        ...
        try {
            ...
            if (win.location.href === loc) {
                unwrap(win.location).reload(location);
            } else {
                win.location.href = loc;
            }
            // END
...
BrowserBot.prototype.modifyWindowToRecordPopUpDialogs = function(originalWindow, browserBot) {
    ...
    // comment to avoid override window methods defined in prompt.js
    ...
    if (browserVersion.isHTA) {
        ...
        if (windowToModify.eval) {
            windowToModify.eval(trustedPolicy.createScript(setOriginalRef));
...
BrowserBot.prototype.close = function() {
    ...
    if (browserVersion.isChrome || browserVersion.isSafari || browserVersion.isOpera) {
        ...
        this.getCurrentWindow().eval(trustedPolicy.createScript("window.top.close();"));
    }
};

BrowserBot.prototype.refresh = function() {
    unwrap(this.getCurrentWindow().location).reload(true);
};
...
SafariBrowserBot.prototype.refresh = function() {
    ...
    if (win.location.hash) {
        ...
        var actuallyReload = function() {
            unwrap(win.location).reload(true);
        };
